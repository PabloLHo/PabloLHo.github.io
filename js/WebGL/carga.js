let particles = [];
let loadingCanvasHypot;

let loadingCanvas, ctx, width, height, width_half, height_half;
const {
    PI,
...
window.addEventListener('load', () => {
    loadingCanvas = loadingCanvas = document.getElementById("loadingCanvas");
    ctx = loadingCanvas.getContext('2d');
    document.body.append(loadingCanvas);
...
function resized() {
    ...
    ({ width, height } = loadingCanvas);
    width_half = width * 0.5;
    height_half = height * 0.5;
    loadingCanvasHypot = Math.hypot(width_half, height_half);
    ctx.beginPath();
...
function draw(e) {
    ...
    ctx.scale(scl, scl);
    ctx.translate(-width_half, -height_half);
    ctx.drawImage(loadingCanvas, 0, 0, width, height);
    ctx.restore();
    ctx.save();
    ctx.beginPath();
...
class Particle {
    ...
    reset(outside) {
        ...
        let r = random(...radiuRange) * loadingCanvasHypot;
